...
uniform internal API** regardless of vendor, so the rest of CASSA never imports a
vendor SDK.

## 1. Standards we build on

| Standard | What it is | Use in CASSA |
|----------|-----------|--------------|
...
| **Vendor SDKs** | ZWO ASI, QHY, FLI, SBIG, PlaneWave PWI, Software Bisque, ToupTek, etc. | Only via an Alpaca/INDI driver wrapper — never called directly by CASSA business logic. |

**Decision:** CASSA defines its own thin **Device Abstraction Layer (DAL)** with one
...
| **ToupTek GEM guide cam** | guide camera | `indi_toupbase` + **PHD2** | USB; PHD2 connects to it as an INDI camera; CASSA talks to PHD2 (see §4) |
...
cannot run the CASSA Site Agent, and cannot be orchestrated from the IUB core. **Do
not build CASSA on the StellaVita app.**
...
- **Edge node = an open Linux box you fully control**: a **Raspberry Pi 5 (8 GB)** or a
  ...
  devices straight into it; run `indiserver` with the drivers above + PHD2. CASSA's
...
- **Quick openness test for StellaVita** (might save buying a separate Pi): from a
  ...
  **open** INDI server and *may* serve as the edge host (if it also lets you install
  ...
  and use your own Linux box. Either way CASSA's code is identical — it only ever sees
...
## 2. Device roles & the operations CASSA needs
